...
//
//    uA_CV_scan[lut_index] = - ADC_CV_array[lut_index] * uA_per_adc_count; // uA read while performing CV scan
//    
//    //send out values with the UART for the CV graph --> TO BE ADDED
//    
//    len= snprintf(str, sizeof(str), "uA ADC read: %.8f\r\n", uA_CV_scan[lut_index]);
//    UART_Debug_PutString(str);
    
    int16 valore_adc_mv_CV= ADC_SigDel_CountsTo_mVolts(ADC_SigDel_GetResult32());
    float current_CV= (float)(-1)*(valore_adc_mv_CV)/20.0;
   
    int16 current_int_CV = (int)(current_CV);
    int8 current_int_CV_H = current_int_CV >> 8 ;
    int8 current_int_CV_L = current_int_CV & (0xFF);
...
//    sprintf(DataBuffer, "A"); //A character is used before every data sent out 
//    UART_PutString(DataBuffer);
//    
//    len= snprintf(str, sizeof(str), "%d", current_int_CV_H);
//    UART_PutString(str);
//    
//    len= snprintf(str, sizeof(str), "%d", current_int_CV_L);
//    UART_PutString(str);
...
CY_ISR(adcAmpInterrupt){
    ...
    uA_amp[lut_index]= (float)(-1)*(valore_adc_mv_AMP)/20.0;
